    ...
    public partial class FSpineAnimator
    {
        ...
        void CalculateTargetBoneRotation(int index)
        {
            ...
            if (GoBackSpeed <= 0f)
            {
                ...
                if (StraightenSpeed > 0f)
                {
                    ...
                    //if (index == 2) Debug.Log(currentBone.TargetStraightenFactor);

                    currentBone.TargetStraightenFactor = currentBone.BoneLength * delta - (previousPos - SpineBones[leadingBoneIndex].ProceduralPosition).magnitude;
        ...
        void RefreshReferencePose()
        {
            ...
            if (LastBoneLeading) // Head bone is last bone in chain
            {
                ...
                if (!HeadAnchor)
                {
                    SpineBones[leadingBoneIndex].ReferencePosition = headBone.SnapshotPosition;
                    SpineBones[leadingBoneIndex].ReferenceRotation = BaseTransform.rotation;
                }
                else
                {
                    SpineBones[leadingBoneIndex].ReferencePosition = headBone.transform.position;
                    SpineBones[leadingBoneIndex].ReferenceRotation = BaseTransform.rotation;
                ...
                for (int i = SpineBones.Count - 2; i >= 0; i--)
                {
                    ...
                    SpineBones[i].ReferencePosition = SpineBones[i + 1].ReferencePosition - (SpineBones[i].ReferenceRotation * ModelForwardAxis * (SpineBones[i].BoneLength * DistancesMultiplier * BaseTransform.lossyScale.x));
                }
            }
